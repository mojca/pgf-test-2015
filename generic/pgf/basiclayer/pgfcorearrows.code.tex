...
\ProvidesFileRCS $Header: /cvsroot/pgf/pgf/generic/pgf/basiclayer/pgfcorearrows.code.tex,v 1.26 2013/09/19 22:55:12 tantau Exp $
...
\pgfdeclarearrow{
  ...
  setup = {
    ...
    \pgfarrowssavethe\pgfutil@tempdimb%
    % The hull:
    \pgfarrowshullpoint{\pgfarrowlength\advance\pgf@x by-.5\pgf@xb\ifpgfarrowharpoon\advance\pgf@x  by1.5\pgf@xa\fi}{\ifpgfarrowharpoon-.5\pgf@xb\else0pt\fi}%
...
\def\pgfarrowsdeclare#1#2#3#4{%
  \pgfdeclarearrow{ name  ={#1}-{#2}, setup ={#3}, code  ={#4}, parameters/.expand once={\csname pgf@arrow@compat@opt@#2\endcsname}}  
...
\def\pgf@arrowsdeclarecombine[#1]#2#3#4#5#6#7{\pgfdeclarearrow{name ={#2}-{#3}, means={#5[sep=#1]#6}}}
...
\def\pgf@arrowsdeclarecombine@star[#1]#2#3#4#5#6#7{\pgfdeclarearrow{name ={#2}-{#3}, means={#5[sep=#1].#6}}}
