...
\ProvidesFileRCS $Header: /cvsroot/pgf/pgf/generic/pgf/basiclayer/pgfcorearrows.code.tex,v 1.15 2013/07/15 12:05:31 tantau Exp $
...
\def\pgf@arrowset#1#2{%
  ...
  {\pgferror{Arrow #1 type ``#2'' unknown}}
  {%
...
\def\pgf@arrows@check@already#1#2#3{%
  \pgfutil@ifundefined{pgf@arrow@code@#2}
  {#3}%
  {\pgferror{Arrow end type ``#2'' already defined}}%
}
...
\def\pgf@nousepath@here#1{%
  \pgferror{The definition of an arrow may not use \string\pgfusepath}
}
    ...
    {%
      ...
      {%
        ...
        \expandafter\def\csname pgf@arrow@left@#3\endcsname{%
          \pgf@process{%
            \edef\pgf@marshal{\noexpand\pgfmathsetlength\pgf@y{#1}}%
            \pgf@marshal%
