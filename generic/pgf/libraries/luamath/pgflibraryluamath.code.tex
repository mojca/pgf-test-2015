...
% Mark's way (modified so that named boxes are ok). To be merged.

\directlua{require('pgf.luamath.parser')}
\directlua{require('pgf.luamath.functions')}

\newif\ifpgfluamathunitsdeclared

\def\pgfluamathparse#1{%
  \pgfluamathunitsdeclaredfalse
  % The following is required for chadef'ed named boxes. The parser
  ...
  \edef\pgfluamath@temp{%
    \directlua{%
      pgf.luamath.parser.parse("\luaescapestring{#1}")
      tex.sprint(parsed_expression)}}%
  % We can now feed back the evaluator.
  \edef\pgfluamathresult{%
    \directlua{%
      pgf.luamath.parser.eval('\pgfluamath@temp')
      if result==nil then
        tex.print('nil')
      else
        tex.print(result)
      end}}%
  \csname pgfluamathunitsdeclared\directlua{%
    if pgf.luamath.parser.units_declared == true then
      tex.print('true')
    else
      tex.print('false')
    end}\endcsname}
