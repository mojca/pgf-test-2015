...
--- @release $Header: /cvsroot/pgf/pgf/generic/pgf/libraries/graphdrawing/lua/algorithms/pgflibrarygraphdrawing-algorithms-spring.lua,v 1.1 2011/05/02 00:59:33 jannis-pohlmann Exp $
...
function drawGraphAlgorithm_spring(graph, options)
  ...
  local max_repulsive_force = graph:getOption('maximum repulsive force') or 6
  ...
  local max_vertex_movement = graph:getOption('maximum iterative node movement') or 0.5
  ...
  -- function to compute the attractive forces of nodes adjacent 
  -- to a given edge
  local function computeAttractiveForce(edge)
  ...
  for i = 1, iterations do
    ...
    for u = 1, #nodes do
      for v = u + 1, #nodes do
        computeRepulsiveForce(nodes[u], nodes[v])
      end
    end

    -- update attractive forces between adjacent nodes
    for edge in keys(graph.edges) do
      computeAttractiveForce(edge)
    end
