...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/tikz/libraries/graphs/tikzlibrarygraphs.code.tex,v 1.27 2012/03/29 21:20:21 tantau Exp $
...
\def\tikz@lib@graph@quotes@slash{%
  \expandafter\endgroup\expandafter\tikz@lib@graph@cont@quote\tikzlibgraphreplaced%
}
    ...
    \expandafter\ifx\csname tikz@lib@graph@def@\tikz@lib@graph@name@only\endcsname\relax%
      ...
      {%
        ...
        {%
          ...
          \else%
            \tikz@child@count@callback{\pgfkeysvalueof{/tikz/graphs/placement/chain count}+1}{\pgfkeysvalueof{/tikz/graphs/placement/element count}+1}%
...
\def\tikz@lg@test@underscore{%
  ...
  \else%
    \expandafter\pgfutil@in@\expandafter{\expandafter/\expandafter}\expandafter{\tikz@lib@graph@name@only}%
    \ifpgfutil@in@%
      \expandafter\tikz@lg@is@slash\tikz@lib@graph@name@only\pgf@stop%
    \else%
      \edef\tikz@lib@graph@name{\tikz@lib@graph@path\tikz@lib@graph@name@only}%
...
\def\tikz@lg@is@underscore#1__{%
  ...
  \fi%  
  \edef\tikz@lib@graph@name{\tikz@lib@graph@path\tikz@lib@graph@name@only}%
  \pgfutil@ifnextchar"\tikz@lg@underscore@quote{\pgfutil@ifnextchar\tikz@active@quotes@token\tikz@lg@underscore@quote@active\tikz@lg@underscore@cont}
}
\def\tikz@lg@underscore@quote"#1"\pgf@stop{%
  \def\tikz@lib@graph@node@text{#1}%
}
{\catcode`\"=13\relax
  \gdef\tikz@lg@underscore@quote@active"#1"\pgf@stop{%
    \def\tikz@lib@graph@node@text{#1}%
  }
}
\def\tikz@lg@underscore@cont#1\pgf@stop{%
  \def\tikz@lib@graph@node@text{#1}%
}

\def\tikz@lg@is@slash#1/{\tikz@lg@is@underscore{#1}__}
