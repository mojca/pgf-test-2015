...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/tikz/libraries/graphs/tikzlibrarygraphs.code.tex,v 1.31 2012/05/14 10:27:46 tantau Exp $
...
\def\tikz@lib@prepare@active#1#2#3{%
  ...
  \g@addto@macro\tikzlibgraphactivations{\catcode`#1=13\relax}  
  \expandafter\g@addto@macro\expandafter\tikzlibgraphactivations\expandafter{\tikzlibgraphactivator}
}
...
\g@addto@macro\tikzlibgraphactivations{\catcode`\\=13\relax}
{
  \catcode`\|=0\relax
  \catcode`\\=13\relax
  |g@addto@macro|tikzlibgraphactivations{|def\{@REVERSE SOLIDUS@}}
}
{
...
}
{
  ...
  \catcode`\<=1\relax%
  \catcode`\>=2\relax%
  \g@addto@macro\tikzlibgraphactivationsbrace<\def{<@LEFT CURLY BRACE@>> %}
  \g@addto@macro\tikzlibgraphactivationsbrace<\def}<@RIGHT CURLY BRACE@>> %{
>
...
\def\tikz@lib@graph@joiner#1#2{%
  ...
  {%
    ...
    % Help graph drawing lib by saving edge options:
    \pgfkeysgetvalue{/graph drawing/edge parameters}\tikz@lib@graph@ep%
    \ifx\tikz@lib@graph@ep\relax\else\ifx\tikz@lib@graph@ep\pgfutil@empty\else%
      % Remember the edge options:
      \expandafter\tikz@lib@graph@add@ep\expandafter{\tikz@lib@graph@ep}%
    \fi\fi%
...
\def\tikz@lib@graph@add@ep#1{%
  \pgfkeysaddvalue{/tikz/graphs/@edges styling}{}{,/graph drawing/edge parameters={#1}}
}
...
\def\tikz@lib@layout@start#1#2\pgf@stop{%
  ...
  % 
  % Tougher: prepare normal node options: 
  % 
  \begingroup%
  \setbox0=\hbox{{%
      \pgfinterruptpath%
        \expandafter\tikzset\expandafter{\tikz@lib@layout@node@options}%
        \pgfkeysgetvalue{/graph drawing/node parameters}\pgf@gd@node@options%
        \global\let\pgf@gd@node@options@smuggle\pgf@gd@node@options%
      \endpgfinterruptpath%
    }}%
  \endgroup%
  % 
  ...
  %
  \expandafter\expandafter\expandafter\scope\expandafter\expandafter\expandafter[%
    \expandafter\tikz@lib@layout@startup@options\tikz@lib@layout@options,%
    /utils/exec=\pgfgdsetrequestcallback\tikz@gd@request@callback,%
...
\def\tikz@lib@layout@startup@options{
  /utils/exec={
    \pgfkeyslet{/graph drawing/node parameters}\pgf@gd@node@options@smuggle,
    \pgfgdsetrequestcallback\pgfutil@gobbletwo},
  /graph drawing/layout node name/.expanded=\ifpgf@lib@graph@empty@layout@node\else\tikz@lib@graph@name\fi,
  /graph drawing/layout node text/.expand once=\tikz@lib@graph@node@text,
  /graph drawing/layout node options/.expand once=\tikz@lib@layout@node@options,
  /tikz/graphs/.cd,
}
