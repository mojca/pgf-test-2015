...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/graphdrawing/core/tikzlayer/tikzlibrarygraphdrawing.code.tex,v 1.9 2012/03/29 19:38:39 tantau Exp $
...
\tikzset{
  graph drawing scope/.style={
    ...
    % 
    % Link into child count callbacks
    % 
    /utils/exec={
      \let\tikz@child@count@callback\tikz@child@count@callback@gd
      \let\tikz@child@group@callback\tikz@child@group@callback@gd
    }
  },
  % Link in...
...
\def\tikz@child@count@callback@gd#1#2{
  \pgfmathtruncatemacro\pgf@gd@tempa{#1}%
  \pgfmathtruncatemacro\pgf@gd@tempb{#2}%
  \pgfkeys{/graph drawing/set logical path head/.expanded=\pgf@gd@tempa/\pgf@gd@tempb;}
}
\def\tikz@child@group@callback@gd{
  \pgfkeys{/graph drawing/push logical path head}
}
