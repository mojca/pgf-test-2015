...
-- @release $Header: /cvsroot/pgf/pgf/generic/pgf/graphdrawing/algorithms/spring/pgfgd-algorithm-Hu2006-spring.lua,v 1.6 2011/05/12 02:10:36 jannis-pohlmann Exp $
...
function drawGraphAlgorithm_Hu2006_spring(graph)
  ...
  local use_coarsening = graph:getOption('/graph drawing/spring layout/coarse') == 'true'
  ...
  if use_coarsening then
    ...
    if coarse_graph:getSize() > 2 then
      hu_spring.apply_forces(coarse_graph.graph, k, C, t, tol, hu_spring.adaptive_step_update)
    end
    ...
    while coarse_graph:getLevel() > 0 do
      ...
      hu_spring.apply_forces(coarse_graph.graph, iterations, use_quadtree, k, C, t, tol, hu_spring.conservative_step_update)
    end
  else
    -- compute a random initial layout for the coarsest graph
    ...
    hu_spring.apply_forces(graph, iterations, use_quadtree, k, C, t, tol, hu_spring.conservative_step_update)
  end
...
function hu_spring.apply_forces(graph, iterations, use_quadtree, k, C, t, tol, step_update_func)
        ...
        for cell in table.value_iter(cells) do
          ...
          if #cell.subcells == 0 then
            ...
            for particle in table.value_iter(cell.particles) do
              ...
              local fr = hu_spring.repulsive_force_quadtree(delta:norm(), 
                                                            particle.amount * particle.mass, 
                                                            k, C)
