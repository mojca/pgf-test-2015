...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/graphdrawing/tex/pgflibrarygraphdrawing.code.tex,v 1.17 2012/06/05 21:09:41 tantau Exp $
...
%
\directlua{
  require 'pgf'                          % Setup the main pgf table
  require 'pgf.gd.lib.cleanupthisstuff'  % Need to get rid of this ASAP!
  require 'pgf.gd.control.TeXInterface'  % Now load the main communication class
}
...
\def\pgfgdendscope{%
    ...
    \directlua{
      pgf.gd.control.TeXInterface.runGraphDrawingAlgorithm()
      pgf.gd.control.TeXInterface.endGraphDrawingScope()
    }
  \endgroup
}
...
% Clusters
% 
% The cluster key allows you to specify that a node or an edge belongs
% to a cluster. Using this key multiple times will cause a node to
% become part of multiple clusters. Using the key without a value will
% cause a unique cluster to be created.
% 
% On the Lua layer, clusters will be automatically "separated",
% meaning that all nodes and edges belonging to the same cluster will
% be collected in a table.

\pgfgddeclareforwardedkeys{/graph drawing}{cluster/.code=}
\pgfkeys{/graph drawing/cluster/.append code=\pgf@gd@handle@cluster{#1}}

\newcount\pgf@gd@clustercount

\def\pgf@gd@handle@cluster#1{%
  \def\pgf@gd@cluster@name{#1}%
  \ifx\pgf@gd@cluster@name\pgfkeysnovalue@text%
    \edef\pgf@gd@cluster@name{clusterPGFINTERNAL\the\pgf@gd@clustercount}%
    \global\advance\pgf@gd@clustercount by 1\relax
  \fi%
  \pgfgdnodeparameter{/graph drawing/cluster table}{'\luaescapestring{\pgf@gd@cluster@name}'}%
  \pgfgdedgeparameter{/graph drawing/cluster table}{'\luaescapestring{\pgf@gd@cluster@name}'}%
}

\pgfgdset{cluster table/.accumulates}
...
% #4 = x position of the node (in pt)
% #5 = y position of the node
% #6 = text
%
% This is an internal function and will be called by the Lua engine

\def\pgfgdgeneratenodecallback#1#2#3#4#5#6{%
  {
    \pgfkeys{#3}
    \pgftransformshift{\pgfqpoint{#4pt}{#5pt}}
    \pgfnode{#2}{\pgfkeysvalueof{/graph drawing/generated
        node/anchor}}{#6}{#1}{\pgfkeysvalueof{/graph drawing/generated
        node/use path}} 
  }
}
