...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/graphdrawing/tex/pgflibrarygraphdrawing.code.tex,v 1.20 2012/07/16 22:09:35 tantau Exp $
...
\def\pgfgdbeginlayout{
  \begingroup
    ...
    \directlua{pgf.gd.control.TeXInterface.setupLayout('sublayout \the\pgf@gd@sublayout@num',\the\pgf@gd@sublayout@stack@height,{\pgfkeysvalueof{/graph drawing/graph parameters}},{\pgfkeysvalueof{/graph drawing/node parameters}})}%
    \pgfgdset{layout node name=,layout node text=,layout node
      options=}%reset.
}
...
%
% Options for layout nodes
% 
% 
% For each (sub)layout, a layout node may be created. This is done
% after the nodes of the layout have been arranged. The idea is that
% the node is "vast" and it encompasses all nodes (and the point
% clouds) that are part of the sublayout. This node will then be used
% as part of the layout containing the sublayout.
% 
% The node is created through the node-creating callback from the Lua
% layer with appropriate options set.
%
% All of the following options are inspected the moment the
% \pgfgdbeginlayout command is used. At this moment, a snapshot is
% taken of these options.
% 
% layout node name = The name of the node. If this is empty, no
% callback will be called and no special node will be created for the
% sublayout. (Although the graph drawing engine will still create an
% internal representing node, but this node is not accessible without
% a name.)
% 
% layout node text = Some text to be displayed as part of the
% node. Normally, this text is not displayed for such "vast" nodes,
% but a shape may use it nevertheless (for instance, one might display
% this text at the top of the node).
% 
% layout node options = Some options for the to-be-created
% node. (Typically, these will be tikz options like "draw" or
...
% 
% layout point cloud = This will be a list of points (without
% separating commas) like "(10pt,20pt)(0pt,0pt)(30pt,40pt)" always in
% this syntax. The list will contain all points inside the
% sublayout. In particular, a bounding box around these points will
% encompass all nodes and bend points of the sublayout.
% The bounding box of this point cloud is guaranteed to be centered on
% the origin.
% 
% layout bounding box width = This will be the width of a bounding box
% (in points with the "pt" suffix).
% 
% layout bounding box height = This will be the height of a bounding box
% (in points with the "pt" suffix).
% 
% layout text = This will be text originally given to the layout node
% text option.
% 
...
% the callback will be a call to \pgfgdlayoutnodecontents. This
% command is empty by default, but feel free to redefine as a
% side-effect of "every layout node".
% 
% 
% All of the "layout..." keys as well as \pgfgdlayoutnodecontents will
% be reset at the beginning of a layout. This ensures that nested
% layouts do not "inherit" things like the layout name from
% surrounding layouts (which would be wrong).
%
% 
% Example:
% 
...
% 
% \pgfgdset{
%   layout node name=vast node,
%   layout node text=Vast Node 1,
%   layout node options={draw, thick}
% }
% \pgfgdbeginlayout
% 
% 
% Then, during the run of the graph drawing engine, at some point the
% following command will be called (with some example point cloud):
% 
% \node [name=vast node,
%        /graph drawing/.cd,
%        layout point cloud={(-20pt,-10pt)(20pt,10pt)},
%        layout bounding box width=40pt,
%        layout bounding box height=20pt,
%        layout text={Vast Node 1},
%        start layout node,
%        draw, thick] {\pgfgdlayoutnodecontents};
%        
%
