...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/graphdrawing/tex/pgflibrarygraphdrawing.code.tex,v 1.4 2012/04/17 10:56:42 tantau Exp $
...
\directlua{
  ...
  if resolvers then
    dofile(assert(resolvers.find_file('pgf.lua', 'tex'), 'could not find pgf.lua'))
    dofile(assert(resolvers.find_file('pgfgd-core-loader.lua', 'tex'), 'could not find pgfgd-core-loader.lua'))
  else
    dofile(assert(kpse.find_file('pgf.lua', 'tex'), 'could not find pgf.lua'))
    dofile(assert(kpse.find_file('pgfgd-core-loader.lua', 'tex'),  'could not find pgfgd-core-loader.lua'))
  end
  % Now load controls
  require 'pgf.gd.control.preload'
  require 'pgf.gd.lib.preload'
  require 'pgf.gd.model.preload'
}
...
\def\pgfgdbeginscope{%
  \begingroup % Protecting scope
    ...
    \directlua{
      pgf.gd.control.TeXInterface:newGraph('\luaescapestring{\pgf@gd@graph@parameters}')
    }%
...
\def\pgfgdendscope{%
    ...
    \directlua{
      ...
      pgf.gd.control.TeXInterface:finishGraph()
    }
  \endgroup
}
...
\def\pgf@gd@steal#1#2{%
  \directlua{pgf.gd.control.TeXInterface:setGraphParameterDefault('\luaescapestring{#1}','\luaescapestring{#2}')}%
}
  ...
  \begingroup%
    \setbox0=\hbox{{%
        \pgfinterruptpath%
          ...
          \toks1={#5}%
          \directlua{
            pgf.gd.control.TeXInterface:addEdge(
...
\pgfgddeclareforwardedkeys{/graph drawing}{
  ...
  spanning tree algorithm/.parameter initial=Spanning Tree Breadth First,
...
\pgfgddeclareforwardedkeys{/graph drawing}{
  ...
  component order/.graph parameter,
  component packing/.graph parameter,
...
\def\pgfgdaddnodetocluster#1#2{
  \directlua{pgf.gd.control.TeXInterface:addNodeToCluster('\luaescapestring{#2}', '\luaescapestring{#1}')}
}
...
\def\pgf@gd@positionnode@callback{%
  {%
    ...
    % call lua system library to create a lua node object
    \directlua{
      pgf.gd.control.TeXInterface:addNode(
