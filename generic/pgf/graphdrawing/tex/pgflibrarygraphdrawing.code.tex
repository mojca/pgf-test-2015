...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/graphdrawing/tex/pgflibrarygraphdrawing.code.tex,v 1.33 2013/03/20 17:16:38 tantau Exp $
  ...
  \pgfutil@for\pgf@temp:=\pgf@list\do{%
    ...
    \else
      \directlua{if not pgf_lookup_and_require('\pgf@temp') then
        tex.print('\luaescapestring{\noexpand\PackageError{pgf}{Graph drawing library '\pgf@temp' not found}{}}')
      end}
    \fi
  }
}
...
\directlua{
  function pgf_lookup_and_require(name)
    local function lookup(name)
      local sub_unix = name:gsub('%.','/')  
      local sub_windows = name:gsub('%.','\string\\')
      if kpse.find_file(sub_unix, 'lua') then
        require(name)
      elseif kpse.find_file(sub_unix, 'clua') then
        collectgarbage('stop') 
        require(name)
        collectgarbage('restart')
      elseif kpse.find_file(sub_windows, 'lua') then
        require(name)
      elseif kpse.find_file(sub_windows, 'clua') then
        require(name)
      else
        return false
      end
