...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/modules/pgfmodulebending.code.tex,v 1.9 2013/09/18 17:13:47 tantau Exp $
...
\def\pgf@draw@curved#1#2{%
  ...
  {%
    \pgfarrows@getid{#1}{#2}%
    \expandafter\ifx\csname pgf@ar@bendable@#1\endcsname\pgf@falsetext\ifdim\pgf@arrows@the@rigidity<0pt\pgf@arrows@the@rigidity-\pgf@arrows@the@rigidity\fi\fi%
    % Do shift:
...
\def\pgf@arrow@drawer@rigid@or@bend#1#2{% rigid start, rigid end
  \ifdim\pgf@arrows@the@rigidity<0pt%
    ...
    \fi%
    {\pgf@relevantforpicturesizefalse\pgfscope\csname pgf@ar@code@\pgf@arrow@id\endcsname\endpgfscope}%
