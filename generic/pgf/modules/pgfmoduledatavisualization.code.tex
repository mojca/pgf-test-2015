...
\ProvidesFileRCS $Header: /cvsroot/pgf/pgf/generic/pgf/modules/pgfmoduledatavisualization.code.tex,v 1.41 2011/02/28 17:53:26 tantau Exp $
...
%   |define set| is read from here and all other keys are ignored
%
...
\def\pgf@dataset@data@opt[#1]{%
  ...
  \begingroup%
    ...
    \pgfkeysgetvalue{/pgf/data/use set}\pgf@dv@use@set%
    \ifx\pgf@dv@use@set\pgfutil@empty%
      \pgf@dv@do@adddata{\pgfkeysvalueof{/pgf/data visualization/obj}.add data(\pgf@do@data{#1})}%
      \expandafter\pgf@dataset@data@opt@cont%
    \else%
      \expandafter\ifx\csname pgf@dv@set@@\pgf@dv@use@set\endcsname\relax%
        \PackageError{pgf}{Undefined data set '\pgf@dv@use@set'}{}%
      \else%
        {%
          \pgfkeys{/pgf/data/define set/.code=}% no redefinitions during use
          \csname pgf@dv@set@@\pgf@dv@use@set\endcsname%
        }%
      \fi%
  \endgroup%  
...
\def\pgf@dataset@grab@external{%
    \fi%
  \endgroup%
  \pgf@dv@do@adddata{\pgfkeysvalueof{/pgf/data visualization/obj}.add data({{{\pgf@dataset@do@external}}})}%
  \pgf@dv@cont@code%
}

\def\pgf@dv@do@adddata#1{%
  \pgfkeysgetvalue{/pgf/data/store in set}\pgf@dv@temp%
  \ifx\pgf@dv@temp\pgfutil@empty%
    #1%
  \else%
    \expandafter\pgfutil@g@addto@macro\csname pgf@dv@set@@\pgf@dv@temp\endcsname{#1}%
  \fi%
}
...
{\catcode`\^^M=\active%
  \gdef\pgf@dataset@grab@@inline#1{%
    \endgroup%
    \pgf@dv@do@adddata{\pgfkeysvalueof{/pgf/data visualization/obj}.add data({{{\pgf@dataset@do@inline#1^^M\pgf@@eol}}})}%
    \fi%
  \endgroup%
  \pgf@dv@cont@code%
  }%
...
\pgfkeys{
  ...
  /pgf/data/store in set/.initial=,
  /pgf/data/new set/.code={\expandafter\global\expandafter\let\csname pgf@dv@set@@#1\endcsname\pgfutil@empty},
  /pgf/data/use set/.initial=,
}
