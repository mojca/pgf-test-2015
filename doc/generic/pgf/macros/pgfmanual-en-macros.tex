...
% $Header: /cvsroot/pgf/pgf/doc/generic/pgf/macros/pgfmanual-en-macros.tex,v 1.72 2013/08/28 15:24:26 tantau Exp $
...
\newenvironment{lualibrary}[1]{
  \pgfmanualentry
  \pgfmanualentryheadline{\textbf{Graph Drawing Library} \texttt{\declare{#1}}}
...
\newenvironment{luadeclare}[4]{
  ...
  \def\manual@temp@{#3#4}%
  \pgfmanualentryheadline{{\ttfamily/graph
...
\newenvironment{luadeclarestyle}[4]{
  ...
  \def\manual@temp@{#3#4}%
  \pgfmanualentryheadline{{\ttfamily/graph
      drawing/\declare{#1}}\ifx\manual@temp@para\pgfutil@empty\else\opt{\texttt=}\opt{#2}\fi\hfill%
...
\newenvironment{shape}[1]{
  \begin{pgfmanualentry}
  	\pgfmanualentryheadline{\textbf{Shape} {\ttfamily\declare{#1}}}%
...
\newenvironment{pictype}[2]{
  \begin{pgfmanualentry}
  	\pgfmanualentryheadline{\textbf{Pic type} {\ttfamily\declare{#1}#2}}%
...
\newenvironment{shading}[1]{
  \begin{pgfmanualentry}
    \pgfmanualentryheadline{\textbf{Shading} {\ttfamily\declare{#1}}}%
...
\newenvironment{graph}[1]{
  \begin{pgfmanualentry}
  	\pgfmanualentryheadline{\textbf{Graph} {\ttfamily\declare{#1}}}%
...
\newenvironment{gdalgorithm}[2]{
  \begin{pgfmanualentry}
  	\pgfmanualentryheadline{\textbf{Layout} {\ttfamily/graph drawing/\declare{#1}\opt{=}}\opt{\meta{options}}}%
    \index{#1@\protect\texttt{#1} layout}%
...
\newenvironment{dataformat}[1]{
  \begin{pgfmanualentry}
  	\pgfmanualentryheadline{\textbf{Format} {\ttfamily\declare{#1}}}%
...
\newenvironment{stylesheet}[1]{
  \begin{pgfmanualentry}
  	\pgfmanualentryheadline{\textbf{Style sheet} {\ttfamily\declare{#1}}}%
...
\def\extracthandler#1#2\@nil{%
  \pgfmanualentryheadline{\textbf{Key handler} \meta{key}{\ttfamily/\declare{#1}}#2}%
...
\newenvironment{ooclass}[1]{
  \begin{pgfmanualentry}
    \def\currentclass{#1}
    \pgfmanualentryheadline{\textbf{Class} \declare{\texttt{#1}}}
...
\newenvironment{predefinednode}[1]{
  \begin{pgfmanualentry}
    \pgfmanualentryheadline{\textbf{Predefined node} {\ttfamily\declare{#1}}}%
...
\newenvironment{coordinatesystem}[1]{
  \begin{pgfmanualentry}
    \pgfmanualentryheadline{\textbf{Coordinate system} {\ttfamily\declare{#1}}}%
...
\newenvironment{pgflibrary}[1]{
  \begin{pgfmanualentry}
    \pgfmanualentryheadline{\textbf{\tikzname\ Library} \texttt{\declare{#1}}}
...
\newenvironment{purepgflibrary}[1]{
  \begin{pgfmanualentry}
    \pgfmanualentryheadline{\textbf{{\small PGF} Library} \texttt{\declare{#1}}}
    \index{#1@\protect\texttt{#1} library}%
    \index{Libraries!#1@\protect\texttt{#1}}%
    \vskip.25em%
    {{\ttfamily\char`\\usepgflibrary\char`\{\declare{#1}\char`\}\space\space\space
...
\newenvironment{arrowtip}[4]{
  \begin{pgfmanualentry}
    ...
    \begin{minipage}[t]{9.5cm}
      #2
    \end{minipage}\hskip5mm\begin{minipage}[t]{5.5cm}
      \leavevmode\vskip-2em
    \tikz{
      \draw [line width=5mm,-{#1[#3]}] (-4,0) -- (0,0);
      \foreach \action in {#4}
...
\def\manual@action@length#1/#2/#3\relax{%
  \draw [red,|<->|,semithick,xshift=#2] (current bounding box.north -| -#1,0) -- node
  [above=-1pt] {|length|} (current bounding box.north -| 0,0);
}
\def\manual@action@width#1/#2/#3\relax{%
  \draw [red,|<->|,semithick] (.5,-#1/2) -- node [below,sloped] {|width|} (.5,#1/2);
}

\newenvironment{arrowexamples}
{\medskip
  \begin{tabbing}
    \hbox to 10cm{\emph{Appearance of the below at line width} \hfil} \= 
     \hbox to 2cm{\emph{0.4pt}\hfil} \= \hbox to 2.5cm{\emph{0.8pt}\hfil} \= \emph{1.6pt} \\
  }
{\end{tabbing}\vskip-1em}

\def\arrowexample[#1]{
  {\sfcode`\.1000\texttt{\currentarrowtype[#1]}} \>
  \tikz [baseline,>={\currentarrowtype[#1]}] \draw [line
  width=0.4pt,->] (0,.5ex) -- (2em,.5ex); thin \>
  \tikz [baseline,>={\currentarrowtype[#1]}] \draw [line
  width=0.8pt,->] (0,.5ex) -- (2em,.5ex); \textbf{thick} \>
  \tikz [baseline,>={\currentarrowtype[#1]}] \draw [line
  width=1.6pt,->] (0,.5ex) -- (2em,.5ex); \\
}
\def\arrowexampledup[#1]{\arrowexample[{#1] \currentarrowtype[}]}
\def\arrowexampledupdot[#1]{\arrowexample[{#1] . \currentarrowtype[}]}

\def\arrowexampledouble[#1]{
  {\sfcode`\.1000\texttt{\currentarrowtype[#1]} on double line} \>
  \tikz [baseline,>={\currentarrowtype[#1]}]
    \draw [double,line width=0.4pt,->] (0,.5ex) -- (2em,.5ex); thin \>
  \tikz [baseline,>={\currentarrowtype[#1]}]
    \draw [double,line width=0.8pt,->] (0,.5ex) -- (2em,.5ex); \textbf{thick} \>
  \tikz [baseline,>={\currentarrowtype[#1]}]
    \draw [double, line width=1.6pt,->] (0,.5ex) -- (2em,.5ex); \\
}
