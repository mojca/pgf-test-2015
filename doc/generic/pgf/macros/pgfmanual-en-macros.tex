...
% $Header: /cvsroot/pgf/pgf/doc/generic/pgf/macros/pgfmanual-en-macros.tex,v 1.65 2012/11/20 23:40:59 tantau Exp $
...
\newenvironment{luatable}[2]{
  \medskip
  \luageneric{#1}{#2}{}{\textbf{Lua table}}
}{\endluageneric}
...
\newenvironment{lualibrary}[1]{
  ...
  \pgfmanualentryheadline{\textbf{Graph Drawing Library} \texttt{\declare{#1}}}
    ...
    {\ttfamily\char`\\usegdlibrary[\declare{#1}]\space \char`\%\space\space Con\TeX t}\\[.5em]
    \pgfmanualbody
	  \pgfmanualpdflabel{#1}{}%
}{\endpgfmanualentry}
...
\newenvironment{luacommand}[3]{
  \luageneric{#1}{#2}{\texttt(#3\texttt)}{\texttt{function}}
}{\endluageneric}
...
\def\parameteritem@[#1]{\def\test{#1}\ifx\test\pgfutil@empty\else\hskip1ex plus 2ex\textbullet\kern1ex#1\kern.5em\fi}
...
\newenvironment{gdalgorithm}[2]{
  \begin{pgfmanualentry}
  	\pgfmanualentryheadline{Layout {\ttfamily/graph drawing/\declare{#1}\opt{=}}\opt{\meta{options}}}%
    \index{#1@\protect\texttt{#1} layout}%
